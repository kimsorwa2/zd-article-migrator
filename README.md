...
- (선택) Vision AI 키 — **AI 설정** 화면에서 DB에 저장 (환경변수 불필요)
  - Gemini API 키
  - OpenAI API 키
...
| **AI 설정** | Vision AI 연동(다중)·연결 테스트·**연동별 OCR 프롬프트** 선택 |
| **프롬프트 관리** | OCR Vision 프롬프트 템플릿 CRUD (시스템·사용자 프롬프트) |
...
- Zendesk 인스턴스(서브도메인, **인스턴스별 OAuth 클라이언트**, 연결) 등록·재연결·삭제
...
**AI 설정**: 동일 제공자라도 계정·API 키별로 여러 **연동 프로필**을 등록합니다. 각 연동 행의 **OCR 프롬프트** 셀렉트로 사용할 템플릿을 지정하고, 「사용」으로 OCR·이미지 변환에 쓸 연동을 선택합니다.

**프롬프트 관리**: 프롬프트 템플릿(시스템·사용자 프롬프트)을 별도 메뉴에서 작성·수정·삭제합니다. OCR 실행 시 **활성 연동에 지정된 프롬프트**가 우선 적용됩니다 (`ai_ocr_connections.prompt_template_id`).
...
AI 설정 UI에서는 foundation model ID를 선택하면 되고, **연결 테스트·OCR 시 자동 변환**됩니다. AI 연동 추가 모달에서 리전별 실제 호출 ID를 미리 볼 수 있습니다.
...
├── frontend/
│   └── src/
│       ├── pages/            # Instances, Migrate, AiOcr, ConvertImage, AiSettings, AiPromptManage, AiOcrMonitor
│       ├── components/       # 트리·진행률·AiOcrConnectionModal·OCR 미리보기
